...
template <typename T, size_t N, size_t M> class Matrix {
  ...
  public:
    ...
    std::array<T, M> &operator[](size_t index) { return data_[index]; }

    const std::array<T, M> &operator[](size_t index) const { return data_[index]; }

    sf::Vector3<T> as_vec(size_t i) const { return {data_[i][0], data_[i][1], data_[i][2]}; }
